  ...
  const content = (
    <span className={`inline-flex items-center gap-2 min-w-0 ${className}`}>
      ...
      <span className="min-w-0">
        ...
        {showLabel && (
          <span className="block text-[10.5px] font-medium text-secondaryText">Team Lead</span>
        )}
      </span>
    </span>
  );

  if (!leadId) return content;

  return (
    <Link
      to={`/dashboard/users/${leadId}`}
